...
{
    public partial class viewTransactionFrontDesk : Form
    {
        ...
        onGoingTransactionFrontDesk _onGoingTransactionFrontDesk;
        public viewTransactionFrontDesk(string customerName, int invoiceID, onGoingTransactionFrontDesk onGoingTransactionFrontDesk, string adminFullname)
        {
            InitializeComponent();
            this.label6.Text = customerName;
            this.label28.Text = invoiceID.ToString();
            ShowCustomerTransaction(customerName, invoiceID);
            _onGoingTransactionFrontDesk = onGoingTransactionFrontDesk;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        // Your code for the "Missing" or "Void" rental status
                        StoreinTransactionHistory();
                    }
                }
            }
                ...
                using (SqlConnection conn = new SqlConnection(con.ConnectionString))
                {
                    ...
                    using (SqlTransaction transaction = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            _onGoingTransactionFrontDesk.ShowOngoingTransaction();
