    ...
    public partial class staffLoginForm : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (count > 0)
                {
                    SqlCommand cmdFullname = new SqlCommand("SELECT employee_fullname FROM empl_info INNER JOIN empl_acc ON empl_info.employee_id = empl_acc.employee_id WHERE empl_acc.employee_username = @Username AND empl_acc.employee_password = @Password AND (empl_info.employee_position = 'Front Desk' OR empl_info.employee_position = 'Operations Supervisor')", con);
                    ...
                    if (!string.IsNullOrEmpty(adminFullname))
                    {
                        ...
                        frontDeskMainPanel frontDeskMainPanel = new frontDeskMainPanel(adminFullname);
                        this.Hide();
                        frontDeskMainPanel.Show();
                    }
                    else
                    {
